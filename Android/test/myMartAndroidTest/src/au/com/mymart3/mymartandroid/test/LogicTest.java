...
//	}
	...
	//Note !!!
	//Not fill all, except QuickPinAuthenticate ----> cannot test b/c User is already register, so don't know what is Pin ID?????
	public void testAuthenticateDeviceQuickPinSucess(){
		
		// tell the mock object what value we expect
		AuthenticateDeviceQuickPinMock authenMock = new AuthenticateDeviceQuickPinMock("{\"AuthenticateDeviceQuickPinJsonResult\":{\"Authenticated\":true,\"ExceptionMessage\":null,\"UserID\":\"bc9ce5ff-1731-457f-bee3-336a99165c22\"}}");
		...
		if(authenModel.authenticated == true){
			assertTrue(true);
		}
		else{
			assertTrue(false);
